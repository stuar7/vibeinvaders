...
export const processPlayerMovement = ({ 
  ...
  const playerPowerUps = gameState.playerPowerUps;
  const movePlayer = gameState.movePlayer;
  const updatePlayerVelocity = gameState.updatePlayerVelocity;
  ...
  // Proper momentum-based dampening (per frame velocity retention)
  // Higher values = keeps more velocity = longer coasting
  // Lower values = loses velocity faster = quicker stopping
  const dampening = freeLookMode ? 0.98 : 0.95; // 98% and 95% velocity retention per frame
